...
namespace Physics
{
	...
	public:
		...
		/// <summary>
		/// If the rotation of the object should be computed to match it's velocity vector
		/// </summary>
		...
		/// </summary>
		Constants::Motion_Type m_type;
		
		/// <summary>
		/// The movement function for the rigidbody;
		/// </summary>
		void(*m_func)(Physics::State& state, double time , double dt);
		...
		void addImpulse(const double x,const double y);
	};
	...
	private:
		...
		static glm::vec2 compute_acceleration(const State& state, double t);
	public:
		/// <summary>
